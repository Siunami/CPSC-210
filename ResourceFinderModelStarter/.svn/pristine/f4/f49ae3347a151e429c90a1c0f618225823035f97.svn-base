...
public class SelectionState {
    ...
    public Set<Resource> getResourcesWithSelectedServices() {
        ...
        if (selected.isEmpty()) {
            resourcesList = registry.getResources();
            for (Resource r : resourcesList) {
                resources.add(r);
            }
        }
        else if (isAnySelected) {
            resources = registry.getResourcesOfferingAnyServicesInSet(selected);
        }
        else {
            resources = registry.getResourcesOfferingAllServicesInSet(selected);
        }
        return resources;
    }
}
